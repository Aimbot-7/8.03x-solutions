...
# Charge amplitude
q0 = (V0 / L) / np.sqrt((omega_0**2 - omega**2)**2 + (gamma * omega)**2)

# Create a PDF file to save the plot
pdf_file = "fig_sol_2.5f.pdf"
with PdfPages(pdf_file) as pdf:
    # Plotting
    plt.figure(figsize=(12, 8))
    plt.plot(omega, q0, label="Charge Amplitude $q_0$", color='b')

    # Titles and labels
    plt.title("Amplitude of Charge $q_0$ vs Angular Frequency $\\omega$", fontsize=18)
    plt.xlabel("Angular Frequency $\\omega$ (rad/s)", fontsize=16)
    plt.ylabel("Amplitude of Charge $q_0$ (C)", fontsize=16)
...
print(f"The plot has been saved to {pdf_file}")
